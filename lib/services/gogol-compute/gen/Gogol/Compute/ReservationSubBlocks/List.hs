...
-- |
-- Module      : Gogol.Compute.ReservationSubBlocks.List
...
--
-- /See:/ <https://cloud.google.com/compute/ Compute Engine API Reference> for @compute.reservationSubBlocks.list@.
module Gogol.Compute.ReservationSubBlocks.List
    (
    -- * Resource
      ComputeReservationSubBlocksListResource

    -- ** Constructing a Request
    , ComputeReservationSubBlocksList (..)
    , newComputeReservationSubBlocksList
    ) where

import qualified Gogol.Prelude as Core
import Gogol.Compute.Types

-- | A resource alias for @compute.reservationSubBlocks.list@ method which the
-- 'ComputeReservationSubBlocksList' request conforms to.
type ComputeReservationSubBlocksListResource =
     "compute" Core.:>
       "v1" Core.:>
         "projects" Core.:>
                                     ...
                                     Core.QueryParam "uploadType" Core.Text Core.:>
                                       Core.QueryParam "upload_protocol" Core.Text Core.:>
                                         Core.QueryParam "alt" Core.AltJSON Core.:>
                                           Core.Get '[Core.JSON] ReservationSubBlocksListResponse

-- | Retrieves a list of reservation subBlocks under a single reservation.
--
-- /See:/ 'newComputeReservationSubBlocksList' smart constructor.
data ComputeReservationSubBlocksList = ComputeReservationSubBlocksList
    {
      -- | V1 error format.
...
-- | Creates a value of 'ComputeReservationSubBlocksList' with the minimum fields required to make a request.
newComputeReservationSubBlocksList 
    :: 
                                   Core.Text
    ...
    -> ComputeReservationSubBlocksList
newComputeReservationSubBlocksList parentName project zone
  = ComputeReservationSubBlocksList{xgafv = Core.Nothing,
                                    accessToken = Core.Nothing, callback = Core.Nothing,
                                    filter = Core.Nothing, maxResults = 500, orderBy = Core.Nothing,
                                    pageToken = Core.Nothing, parentName = parentName,
                                    project = project, returnPartialSuccess = Core.Nothing,
                                    uploadType = Core.Nothing, uploadProtocol = Core.Nothing,
                                    zone = zone}
instance Core.GoogleRequest ComputeReservationSubBlocksList where
        type Rs ComputeReservationSubBlocksList =
             ReservationSubBlocksListResponse
        type Scopes ComputeReservationSubBlocksList =
             '[CloudPlatform'FullControl, Compute'FullControl, Compute'Readonly]
        requestClient ComputeReservationSubBlocksList{..}
          = go project zone parentName xgafv accessToken callback filter
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ComputeReservationSubBlocksListResource)
                      Core.mempty
