...
-- |
-- Module      : Gogol.Compute.ReservationBlocks.List
...
--
-- /See:/ <https://cloud.google.com/compute/ Compute Engine API Reference> for @compute.reservationBlocks.list@.
module Gogol.Compute.ReservationBlocks.List
    (
    -- * Resource
      ComputeReservationBlocksListResource

    -- ** Constructing a Request
    , ComputeReservationBlocksList (..)
    , newComputeReservationBlocksList
    ) where

import qualified Gogol.Prelude as Core
import Gogol.Compute.Types

-- | A resource alias for @compute.reservationBlocks.list@ method which the
-- 'ComputeReservationBlocksList' request conforms to.
type ComputeReservationBlocksListResource =
     "compute" Core.:>
       "v1" Core.:>
         "projects" Core.:>
                                       ...
                                       Core.QueryParam "uploadType" Core.Text Core.:>
                                         Core.QueryParam "upload_protocol" Core.Text Core.:>
                                           Core.QueryParam "alt" Core.AltJSON Core.:>
                                             Core.Get '[Core.JSON] ReservationBlocksListResponse

-- | Retrieves a list of reservation blocks under a single reservation.
--
-- /See:/ 'newComputeReservationBlocksList' smart constructor.
data ComputeReservationBlocksList = ComputeReservationBlocksList
    {
      -- | V1 error format.
...
-- | Creates a value of 'ComputeReservationBlocksList' with the minimum fields required to make a request.
newComputeReservationBlocksList 
    :: 
                                Core.Text
    ...
    -> ComputeReservationBlocksList
newComputeReservationBlocksList project reservation zone
  = ComputeReservationBlocksList{xgafv = Core.Nothing,
                                 accessToken = Core.Nothing, callback = Core.Nothing,
                                 filter = Core.Nothing, maxResults = 500, orderBy = Core.Nothing,
                                 pageToken = Core.Nothing, project = project,
                                 reservation = reservation, returnPartialSuccess = Core.Nothing,
                                 uploadType = Core.Nothing, uploadProtocol = Core.Nothing,
                                 zone = zone}
instance Core.GoogleRequest ComputeReservationBlocksList where
        type Rs ComputeReservationBlocksList =
             ReservationBlocksListResponse
        type Scopes ComputeReservationBlocksList =
             '[CloudPlatform'FullControl, Compute'FullControl, Compute'Readonly]
        requestClient ComputeReservationBlocksList{..}
          = go project zone reservation xgafv accessToken callback filter
          ...
          where go
                  = Core.buildClient
                      (Core.Proxy :: Core.Proxy ComputeReservationBlocksListResource)
                      Core.mempty
