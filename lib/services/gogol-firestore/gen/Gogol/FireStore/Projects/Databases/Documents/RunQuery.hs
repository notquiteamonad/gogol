...
-- |
-- Module      : Gogol.FireStore.Projects.Databases.Documents.RunQuery
...
instance
  ...
  requestClient FireStoreProjectsDatabasesDocumentsRunQuery {..} =
    go
      parent
      xgafv
      accessToken
    ...
    where
      go =
        Core.buildClient
          ( Core.Proxy ::
              Core.Proxy FireStoreProjectsDatabasesDocumentsRunQueryResource
          )
          Core.mempty
